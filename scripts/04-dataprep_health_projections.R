...
# get 2020 census population by tract
dc2020 <- readxl::read_excel("data/population/Demographic_Raw_Tables.xlsx", sheet = 1)%>%
  select(1:3)%>%
  rename(pop = 3)

dc2020_ct_breakdown <- dc2020 %>%
  group_by(County)%>%
  summarise(county_pop = sum(pop, na.rm = T))%>%
  right_join(dc2020, by = "County")%>%
  mutate(pop_county_pct = pop/county_pop)%>%
  select(FIPS, County, pop_county_pct)

county_age_incidence <- county_pop_projections %>%
  filter(Year >= 2018 & Year <= 2023) %>%
  group_by(age_group, lower_age, upper_age, County)%>%
  summarise(county_pop = mean(pop, na.rm = T)) %>%
  ungroup %>%
  left_join(county_incidence_age1yr, by = "County", relationship = "many-to-many")%>%
  filter(as.numeric(age) <= upper_age & as.numeric(age) >= lower_age)%>%
  group_by(County, upper_age, lower_age, county_pop)%>%
  summarise(Deaths = sum(Deaths, na.rm = T))%>%
  mutate(Deaths_annual = Deaths/6,
         incidence_rate = Deaths_annual/county_pop)%>%
  ungroup %>%
  select(County, upper_age, lower_age, incidence_rate)

  
county_incidence_projections <- county_pop_projections %>%
  left_join(county_age_incidence, by = c("County", "lower_age", "upper_age"))

ct_pop_projections <- dc2020_ct_breakdown %>%
  left_join(county_age_incidence, by = "County", relationship = "many-to-many")%>%
  left_join(county_pop_projections, by = c("County", "lower_age", "upper_age"))%>%
  mutate(pop = pop_county_pct*county_pop)
